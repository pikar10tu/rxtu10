...
17. **อีโมจิเสิร์ฟเป็น WebP (`public/emoji/fluent/<cp>.webp` 256px) — SVG เป็นแค่ต้นฉบับ ห้ามให้แอปชี้กลับไป `.svg`**
   iPhone Safari raster SVG ใหม่ทุกครั้งที่ layer ที่มีมันถูกวาดใหม่ (การ์ดรีเพลย์ repaint ทุกหมัด)
   = ต้นเหตุรีเพลย์กระตุก: ห้องเทียบ 26 ก.ย. 2026 <30fps **119+ → 7 เฟรม/ไฟต์** แค่สลับ SVG → WebP
   เพิ่มอีโมจิใหม่: `node scripts/fetch-fluent.mjs` แล้ว **ต้อง** `node scripts/fluent-webp.mjs` ต่อ
   (`emoji.test.js` แดงถ้ามี SVG ที่ไม่มี WebP คู่) · ทุกที่ต้องได้ path จาก `fluentFile()` เท่านั้น ห้ามต่อสตริง path เอง
