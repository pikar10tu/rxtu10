...
เพิ่ม CSS (**FIX fable:** เกรด badge วาง **bottom-left** ไม่ทับ `.pt-cell-el` ที่อยู่ top-left · team badge top-right):
```css
...
```
> หมายเหตุ: **อ่าน PetsView template จริงก่อน** — element chip อยู่ `.pt-cell-el` top-left (PetsView ~:131), stars `.pt-cell-stars` (:46), ⭐ทีม `.pt-cell-star` (:41) · ยืนยันมุมตอน dev ว่า grade(ล่างซ้าย)/team(บนขวา)/element(บนซ้าย) ไม่ทับกัน
...
```
โดย `gradeNow = computed(() => pet.value?.grade || 0)` (เพิ่มถ้ายังไม่มี) · `canUp`/`busy`/`evolve`/`upCost` = ของเดิม (upCost มีอยู่ ใช้ที่ pd-note เดิม) · `MAX_GRADE` — ตรวจว่า import จาก petGrade.js แล้ว (fable: DetailModal import อยู่แล้ว ~:63) ถ้ายังให้เพิ่ม · ลบ markup เกรด/rarity tag/ปุ่มวิวัฒน์+pd-note ตำแหน่งเก่าออก (กันซ้ำ — ย้ายมารวมในพาเนล)
...
**หมายเหตุจาก fable plan-review:**
- **expWeight ไม่ clamp = ถูกต้อง (ยึด plan)** — spec เขียน clampGrade คลาดเคลื่อน แต่ผลเท่ากันเพราะเกรดจริง ≤5 เสมอ · **implementer ยึด plan** (petPower.expWeight ไม่ clamp เพื่อ match partyPower เป๊ะ)
- **petMigration.js `g*5/12` + pvpBot.js `t*5` (literal 5) = defer** — spec เอ่ยถึง MAX_GRADE จุดเดียว แต่ literal พวกนี้อยู่ใน context เฉพาะ (migration scale/bot-gen) + petMigration มีเทส pin → centralize เสี่ยงกว่าคุ้ม · **ไม่แตะใน P1** (gameplay-neutral, เพดานเกรดจริงยังมาจาก array length ที่ petPower)
