 ...
 *  - ชั้น 70–100: เอน theme → "เคาน์เตอร์" (ELS[(t+2)%3]) คือคำตอบ "เสมอ" + สลับ composition รายชั้น
 *    RPS: ELS[k] ชนะ ELS[(k+1)%3] แพ้ ELS[(k+2)%3] · mirror (ธาตุเดียวกัน) = neutral
 *    ⚠️ ห้ามใส่ victim (ELS[(t+1)%3]) เป็นสล็อต — victim ชนะ counter → "ก๊อป theme" ชนะแทน (sim พิสูจน์แล้ว)
 */
function floorElements(f, count) {
  ...
  const t = f % 3
  const theme   = ELS[t]              // ธาตุหลักของชั้น
