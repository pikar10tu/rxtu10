...
import { auth, db, provider, ADMIN_EMAIL, SNAPSHOT_DELAY, PLE_CC_DATE,
         INSTANT_COST, MEGA_COST, VANCO_KE_SLOPE, VANCO_KE_INT, VANCO_VD_POP,
         CG_MALE_FACTOR, CG_FEMALE_FACTOR, CG_DENOMINATOR, IS_DEV, log } from './config.js';
...
async function _ctGameOver(isMiss){
    ...
    const isHigh  = score > (userData?.ctHigh||0);
    ...
    if(currentUser && userData){
        try{
            const updates = { coins: (userData.coins||0)+coins };
            if(isHigh) updates.ctHigh = score;
            await updateDoc(doc(db,'users',currentUser.uid), updates);
            if(isHigh && userData?.studentId && window.__fbUsers?.[userData.studentId])
                window.__fbUsers[userData.studentId].ctHigh = score;
            if(score>30){
                const ctName=userData?.nickname||userData?.name||'ผู้เล่น';
                await postNews("🟩",`${ctName} ทำคะแนน ${score} ในเกม Color Tiles`).catch(()=>{});
            }
