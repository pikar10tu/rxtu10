...
export function buildBeats(log, maxHpByUid, { rng = null, showPets = null, hitSpread = HIT_SPREAD } = {}) {
  ...
  //
  // ✅ P2c-2 (10 ก.ย. 2026) — ข้อจำกัดเดิมตรงนี้ปิดแล้ว: เอนจินบันทึกการตายเงียบ (หนาม · guardian ·
  //    aoeOpener) ลง log เป็นใบ attack แบบ `silent: true` ที่แบก `dead` ครบแล้ว (สเปก 2026-09-10)
  //    ⇒ ไฟต์ที่จบด้วยการตายเงียบไม่มีใบท้าย dead:false ค้างอีกต่อไป และ finishAt ชี้ใบที่ปิดจริงเสมอ
  //    (ด่านวัด: `node scripts/death-audit.mjs` ต้องได้ 0 ไฟต์)
  //
  // `endsFight` = log ก้อนนี้เป็นไฟต์ที่จบจริงหรือเปล่า · เอนจินใส่ `end` ให้ทุกไฟต์เสมอ
  // ใช้เป็นเงื่อนไขของกฎ "อะไรที่อยู่หลังใบปิดเกมต้องเงียบ" (หนี้ §7.6 ข้อ 5) — log ท่อนเดียวจาก
  // เทส/เครื่องมือที่ไม่มี `end` ไม่มีสิทธิ์ตัดสินว่าอะไรอยู่ "หลังไฟต์จบ"
  const endsFight = evts.some(e => e && e.t === 'end')
  let finishAt = -1
  ...
  const openCut = openCutOf(evts)
  {
    ...
    for (let i = openCut; i < evts.length; i++) {
      ...
      // ⚠️ ต้องมี `end` ใน log ด้วย: finishAt คือ "ใบปิดเกม" ก็ต่อเมื่อ log เป็นไฟต์ที่จบจริง
      //    ถ้าไม่มี end (log ท่อนเดียวจากเทส/เครื่องมือ) มันเป็นแค่ "หมัดสุดท้ายเท่าที่เห็น"
      //    ⇒ ไม่มีสิทธิ์ตัดสินว่าอะไร "หลังไฟต์จบ" · เอนจินจริงใส่ end ให้ทุกไฟต์เสมอ
      if (endsFight && finishAt >= 0 && i > finishAt) { pKind.set(i, 'skillQuiet'); groupId = null; groupEffCount = null; continue }
      const uid = e.uid || ''
