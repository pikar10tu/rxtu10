...
test('getTowerBonus: monotonic + ขอบ ladder', () => {
  ...
  assert.equal(getTowerBonus(1), 50)    // มีโบนัสตั้งแต่ชั้น 1
  assert.equal(getTowerBonus(2), 50)
  assert.equal(getTowerBonus(3), 120)
  assert.equal(getTowerBonus(5), 250)
  assert.equal(getTowerBonus(9), 250)   // ยังไม่ถึง 10 → คงค่าชั้น 5
