...
1. **อีโมจิยังเป็นสี่เหลี่ยม (tofu) บางจุด** — พบที่ **"เก็บเหรียญรายวัน"** (Home / `useDaily`)
   → emoji ที่ยังไม่ได้แปลงเป็น `<Emoji>` (น่าจะอยู่ใน JS expression / toast ที่ตั้งใจปล่อยไว้ตอนทำ emoji track)
   ต้องไล่หา emoji ในจุดเก็บเหรียญรายวันแล้วแปลง/เพิ่มไฟล์ Fluent ที่ขาด
2. **ฟาร์ม: ปลูกพืชแล้วเลื่อนลงมาดูผลผลิตไม่ได้** — หน้า/โมดัลฟาร์ม scroll ลงไม่สุด เห็นผลผลิตไม่ครบ
   → น่าจะเป็นบั๊ก overflow/height ของ container (`FarmView` / `useFarm` UI)
